...
std::string DummyHardwareInterface::readSerialResponse()
{
    ...
    try {
        // Read until newline character '\n' (adjust if Arduino uses different termination)
        // This is a blocking read with a default timeout. Configure timeout if needed.
        std::vector<uint8_t> buffer;
        serial_driver_->port()->receive_until(buffer, '\n'); // Adjust delimiter if needed

        if (buffer.empty()) {
             RCLCPP_WARN(rclcpp::get_logger("DummyHardwareInterface"), "Read empty response or timed out.");
             return "";
        }

        std::string response(buffer.begin(), buffer.end());
        RCLCPP_DEBUG(rclcpp::get_logger("DummyHardwareInterface"), "Received response: %s", response.c_str());
        // Remove potential trailing newline/carriage return
        response.erase(response.find_last_not_of("\r\n") + 1);
        return response;

    } catch (const std::exception& e) {
        RCLCPP_ERROR(rclcpp::get_logger("DummyHardwareInterface"), "Error reading serial response: %s", e.what());
        return "";
    }
}
  ...
  const hardware_interface::HardwareInfo & info)
{
  ...
  // Can customize if needed: drivers::serial_driver::FlowControl::NONE, drivers::serial_driver::Parity::NONE, drivers::serial_driver::StopBits::ONE
  ...
  // Set device name after creating the object

  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  try {
      if (!serial_driver_->port()->is_open()) {
          ...
          auto start_open = std::chrono::steady_clock::now(); // Add timing start
          serial_driver_->port()->open();
          auto end_open = std::chrono::steady_clock::now(); // Add timing end
          RCLCPP_INFO(rclcpp::get_logger("DummyHardwareInterface"), "Serial port open() call took %ld ms.", std::chrono::duration_cast<std::chrono::milliseconds>(end_open - start_open).count());

          // --- THIS IS A SIGNIFICANT DELAY ---
          RCLCPP_INFO(rclcpp::get_logger("DummyHardwareInterface"), "Waiting 2.5 seconds after port open...");
          ...
          RCLCPP_INFO(rclcpp::get_logger("DummyHardwareInterface"), "Starting wait loop for ready message..."); // Add log
          while (std::chrono::steady_clock::now() - start_time < timeout) {
              line = readSerialResponse(); // This function has its own 500ms timeout
              if (!line.empty()) {
              ...
              // Add a small sleep here if readSerialResponse returns immediately empty often
              // else { std::this_thread::sleep_for(std::chrono::milliseconds(10)); }
          }
          auto end_wait = std::chrono::steady_clock::now(); // Add timing end
          RCLCPP_INFO(rclcpp::get_logger("DummyHardwareInterface"), "Wait loop finished after %ld ms.", std::chrono::duration_cast<std::chrono::milliseconds>(end_wait - start_time).count());
